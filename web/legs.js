...
(function (root, factory) {
  ...
  // path: [{kind:"board"|"take"|"ride", stop, route, xtype, xdist}]
  //  - "board": penanda asal (route null) — diabaikan; "take" pertama bawa stop sama.
  //  - "take" : naik route (leg baru). xtype = jenis transfer masuk leg ini (s/o/w). xdist = jarak (m).
  //  - "ride" : maju 1 halte di route sama.
  // Return: [{route, xtype, xdist, board, mid:[...halte lewat], alight}]
  function pathToLegs(path) {
    const raw = [];
    for (const s of path) {
      if (s.kind === "take") raw.push({ route: s.route, xtype: s.xtype, xdist: s.xdist, board: s.stop, rides: [] });
    ...
    return raw.map(({ route, xtype, xdist, board, rides }) => ({
      route, xtype, xdist, board,
