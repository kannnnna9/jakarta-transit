...
def find(origin, dest, data):
    stop_name, name_stops, rname, ride, routes_at = data
    ...
    seq = count()
    pq = [(0, 0, next(seq), s, None, [("board", s, None)]) for s in origins]
    heapq.heapify(pq)
        ...
        if route is not None:
            for nxt in ride[route].get(stop, ()):
                heapq.heappush(pq, (tr, st + 1, next(seq), nxt, route,
                                    path + [("ride", nxt, route)]))
        # board / transfer: any route at any same-named stop
        for s2 in name_stops[stop_name[stop]]:
...
def render(res, data):
    ...
    cur_route = None
    for kind, stop, route in path:
        nm = stop_name[stop]
        if kind == "board":
            print(f"START at {nm}")
        elif kind == "take":
            cur_route = route
            print(f"  -> board  [{rname[route]}]  at {nm}")
        elif kind == "ride":
            print(f"       ...  {nm}")
